...
export const useBurnMutation = () => {
    return useMutation({
        ...
        }) => burn({ userAddress, network, vault, amount, walletClient }),
        onSuccess: (data) => showSuccessToast(data, 'Burn'),
...
const burn = async ({
    ...
    const mintTx = await pool.buildBurnTransaction({
        shares: amount,
        vault,
    });
